...
const SEED_PROBLEMS = [
  {
    ...
    solutions: [
      ...
      {
        id: "sol-1-2",
        author: "Helpingo AI Agent",
        text: "🚨 Emergency Water Logging Protocol: \n1. Do not wade through floodwaters as they might be electrically charged or contaminated.\n2. We have logged this report to the municipal helpline (#44921).\n3. Use the flyover bypass route.",
...
app.post('/api/chat/ai', async (req, res) => {
  try {
    ...
    if (GEMINI_API_KEY && GEMINI_API_KEY !== 'your_gemini_api_key_here') {
      try {
        let problemContext = "No specific problem is currently selected. Answer generally.";
        if (problem) {
          problemContext = `
The user has selected the following reported issue in the neighborhood:
- Title: "${problem.title}"
- Description: "${problem.description}"
- Category: "${problem.category}"
- Reported By: "${problem.author}"
- Status: "${problem.status}"
- Location Coordinates: Latitude ${problem.location?.lat}, Longitude ${problem.location?.lng}
- Existing solutions/comments proposed so far: ${JSON.stringify(problem.solutions || [])}
`;
        }

        const prompt = `You are the Helpingo AI Coordinator. Your role is to help neighbors coordinate community aid, answer safety questions, and solve local problems.

User Query: "${query}"

Context about the selected problem:
${problemContext}

Instructions:
1. Respond accurately, helpfully, and concisely.
2. Use clean Markdown formatting. 
3. Keep your response relevant to the user query and the problem context if provided.
4. Keep the tone friendly, practical, and community-minded.
5. If the category is "Emergency", prioritize safety guidelines first.
`;
        ...
        if (apiResponse.ok) {
          ...
          console.warn("Gemini API returned error response:", errText);
        }
      } catch (err) {
        console.error("Failed to fetch from Gemini API. Falling back to local agent:", err.message);
      }
    }

    // Local Smart NLP Agent Fallback
    const replyText = localNLPAgent(query, problem);
    
    // Add a tiny artificial delay to simulate AI thinking
    setTimeout(() => {
      res.json({ reply: replyText });
    }, 600);

  } catch (error) {
    res.status(500).json({ message: "AI agent query failed", error: error.message });
  }
});

// Helper for local smart agent responses
function localNLPAgent(query, problem) {
  const lowerQuery = (query || "").toLowerCase();
  
  if (!problem) {
    // General chat queries
    if (lowerQuery.includes("hello") || lowerQuery.includes("hi") || lowerQuery.includes("hey")) {
      return "Hello! I am the **Helpingo AI Assistant**. I can help you coordinate volunteer efforts, view safety guidelines, or analyze community issues. Click on any problem card in the feed to ask about a specific problem!";
    }
  ...
  // Queries related to a specific problem
  ...
  // 5. Volunteering / How to help / What to do
  ...
  // 6. Generic/default response incorporating the problem details
